...
class _SessionCreateState extends State<SessionCreate> {
  ...
  void _saveSession(BuildContext context) {
    ...
    context.read<UsersCubit>().addSession(newSession, widget.userModel);
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'Описание (опционально)', border: OutlineInputBorder()),
                maxLines: 3,
              ),
              const SizedBox(height: 16),
              TextFormField(
                controller: _durationController,
                decoration: const InputDecoration(labelText: 'Длительность (минуты)', border: OutlineInputBorder()),
              ...
              DropdownButtonFormField<SessionStatus>(
                ...
                items: SessionStatus.values.map((status) {
                  return DropdownMenuItem(value: status, child: Text(status.text));
                }).toList(),
                onChanged: (value) {
                  setState(() {
